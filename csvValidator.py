    ...
    if(keyslen == columnnameslen) :
      ...
      for i in range(columnnameslen):
        try:
          ...
          columnErrors.append(err)
    print(columnErrors)
    with open("./csvproduced/columnErrors.txt", 'w') as the_file:
...
def preprocess_segregate_error_rows(columns, columnnames):
    ...
    with open(sourceCSVfileName, 'r') as csv_file: 
        ...
        print("sourceCSVfileNameNoExt just before");
        print(sourceCSVfileNameNoExt);
        print("(((((((((((((()))))))))")
        print(f'./csvproduced/clean_rows_{sourceCSVfileNameNoExt}_preprocessed.csv')
        writeRowsIntoCSV(clean_rows, f'./csvproduced/clean_rows_{sourceCSVfileNameNoExt}_preprocessed.csv')
        writeRowsIntoCSV(problem_rows, errorFolderPath + "/" + f'problem_rows_{sourceCSVfileNameNoExt}_preprocessed.csv')

def removeTildas():
  print(f'./csvproduced/{sourceCSVfileNameNoExt}_clean_data_intermediate.csv')
...
def do_validation(columnsFromJSON):
    ...
    # # read the data
    try:
      data = pd.read_csv(f'./csvproduced/clean_rows_{sourceCSVfileNameNoExt}_preprocessed.csv', sep=delimiter  , engine='python')
